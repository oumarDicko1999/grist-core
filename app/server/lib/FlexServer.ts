...
export function getGristHost() {
  return process.env.GRIST_HOST || "localhost";
}
...
export class FlexServer implements GristServer {
  ...
  constructor(
    ...
    const ikadocAdmissionConfig = readIkaDocAdmissionConfig();
  ...
  public async addSetupGate() {
    ...
    gateRouter.use(
      expressWrap(async (req, res, next) => {
        ...
        if (isApi) {
          throw new ApiError(
            "Grist is not yet configured. Visit /boot to set up this installation.",
            503,
          );
        }
  ...
  public async addAssetsForPlugins() {
    ...
    this.app.use(
      ...
      limitToPlugins(
        this,
        express.static(getAppPathTo(this.appRoot, "static")),
      ),
    );
    this.app.use(
      "/plugins/assets",
  ...
  public addHomeApi() {
    ...
    this.app.get(
      ["/api/session/access/active", "/api/session/access/all"],
      createIkaDocRuntimeSessionAccessHandler(
        this._ikadocRuntimeSessionRegistry,
        this._ikadocEditorAdmissionClient,
        this._ikadocForwardAuthSecret,
    ...
    this.app.use(
      ...
      denyIkaDocRuntimeOperation(
        this._ikadocRuntimeSessionRegistry,
        "use Grist profile management",
      ),
    );
    this.app.use(
      ["/api/orgs", "/api/workspaces", "/api/templates", "/api/widgets"],
      denyIkaDocRuntimeOperation(
        this._ikadocRuntimeSessionRegistry,
        "browse Grist home resources",
      ),
    );
  ...
  public async addLandingPages() {
    ...
    attachAppEndpoint({
      ...
      gristServer: this,
      ikadocEditorAdmissionClient: this._ikadocEditorAdmissionClient,
      ikadocRuntimeSessionRegistry: this._ikadocRuntimeSessionRegistry,
    });
  }

  public async addLoginMiddleware() {
    if (this._check("loginMiddleware", "homedb")) {
      return;
    }
  ...
  public addComm() {
    ...
    this._comm = new Comm(this.server, {
      ...
      ikadocRuntimeSessionRegistry: this._ikadocRuntimeSessionRegistry,
      ikadocEditorAdmissionClient: this._ikadocEditorAdmissionClient,
      ikadocForwardAuthSecret: this._ikadocForwardAuthSecret,
    });
  }
  ...
  public async addDoc() {
    ...
    const docWorker = new DocWorker(this._dbManager, {
      ...
      tracker,
      ikadocRuntimeSessionRegistry: this._ikadocRuntimeSessionRegistry,
      ikadocRuntimeSessionValidator: this._ikadocRuntimeSessionValidator,
    });
  ...
  public addAccountPage() {
    ...
    const denyIkaDocAccountAccess = denyIkaDocRuntimeOperation(
      this._ikadocRuntimeSessionRegistry,
      "use Grist account management",
    );

    this.app.get(
      "/account(/developer|/authorized-apps)?",
      ...middleware,
      denyIkaDocAccountAccess,
      expressWrap(async (req, resp) => {
  ...
  public addGoogleAuthEndpoint() {
    ...
    log.info(
      "Configured OAuth2 clients: %s",
      this._oauth2Clients.getValidClients(),
    );
  ...
  public addExtraDocForwarder() {
    if (this._check("extraDocForwarder")) {
      return;
    }
  ...
  private async _redirectToHomeOrWelcomePage(
    ...
    const orgs = this._dbManager.unwrapQueryResult(
      await this._dbManager.getOrgs(getScope(mreq), {
